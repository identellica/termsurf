...
fn create_app_layout(app_path: &Path) -> PathBuf {
    [EXEC_PATH, RESOURCES_PATH, FRAMEWORKS_PATH].iter()
...
fn create_info_plist(contents_path: &Path, exec_name: &str) -> Result<(), Box<dyn std::error::Error>> {
...
// TODO: only bundle macos app now
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let app_path = PathBuf::from("target/debug/examples");
