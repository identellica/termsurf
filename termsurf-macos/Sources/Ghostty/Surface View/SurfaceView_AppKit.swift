...
extension Ghostty {
  ...
  class SurfaceView: OSView, ObservableObject, Codable, Identifiable {
    ...
    @objc private func ghosttyDidChangeReadonly(_ notification: SwiftUI.Notification) {
      guard let value = notification.userInfo?[SwiftUI.Notification.Name.ReadonlyKey] as? Bool
      else { return }
      readonly = value
    }
    ...
    override func keyDown(with event: NSEvent) {
      ...
      Ghostty.logger.info(
        "SurfaceView.keyDown: chars='\(chars)' keyCode=\(keyCode) mods=\(event.modifierFlags.rawValue)"
      )
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
      ...
      if let container = subviews.last(where: { $0 is WebViewContainer }) as? WebViewContainer {
        ...
        if hasCmd && hasOpt && char == "i" {
          ...
          if let inspector = container.webViewOverlay.webView.value(forKey: "_inspector")
            as? NSObject
          {
