...
class SurfaceScrollView: NSView {
  ...
  init(contentSize: CGSize, surfaceView: Ghostty.SurfaceView) {
    ...
    // Note: Cursor handling is done via SurfaceView.resetCursorRects() instead of
    // documentCursor, allowing subviews (like WebViewContainer) to manage their own cursors.
  }
  ...
  private func handleScrollbarUpdate(_ notification: Notification) {
    guard
      let scrollbar = notification.userInfo?[SwiftUI.Notification.Name.ScrollbarKey]
