  ...
  class App: ObservableObject {
    ...
    func resetTerminal(surface: ghostty_surface_t) {
      ...
      static func writeClipboard(
        ...
        assert(
          contentArray.filter({ $0.mime == "text/plain" }).count <= 1,
        ...
        -> Bool
      {
        ...
        case GHOSTTY_ACTION_TOGGLE_TAB_OVERVIEW, GHOSTTY_ACTION_TOGGLE_WINDOW_DECORATIONS,
          GHOSTTY_ACTION_SIZE_LIMIT, GHOSTTY_ACTION_QUIT_TIMER, GHOSTTY_ACTION_SHOW_CHILD_EXITED:
      ...
      private static func openURL(
        ...
        case .text:
          ...
          let editor =
            NSWorkspace.shared.defaultApplicationURL(forExtension: url.pathExtension)
            ?? NSWorkspace.shared.defaultTextEditor
          if let textEditor = editor {
            NSWorkspace.shared.open(
              [url], withApplicationAt: textEditor, configuration: NSWorkspace.OpenConfiguration())
            return true
          }
      ...
      private static func closeTab(
        _ app: ghostty_app_t, target: ghostty_target_s, mode: ghostty_action_close_tab_mode_e
      ) {
        switch target.tag {
        case GHOSTTY_TARGET_APP:
      ...
      private static func gotoSplit(
        ...
        case GHOSTTY_TARGET_SURFACE:
          ...
          guard let targetNode = controller.surfaceTree.root?.node(view: surfaceView) else {
            return false
          }
          ...
          let focusDirection: SplitTree<Ghostty.SurfaceView>.FocusDirection =
            splitDirection.toSplitTreeFocusDirection()
          guard controller.surfaceTree.focusTarget(for: focusDirection, from: targetNode) != nil
          else {
            return false
          }
      ...
      private static func gotoWindow(
        ...
        let startIndex =
          candidates.firstIndex(where: { $0.isKeyWindow })
      ...
      private static func resizeSplit(
        ...
        case GHOSTTY_TARGET_SURFACE:
          guard let surface = target.target.surface else { return }
          guard let surfaceView = self.surfaceView(from: surface) else { return }
          guard let resizeDirection = SplitResizeDirection.from(direction: resize.direction) else {
            return
          }
          NotificationCenter.default.post(
      ...
      private static func promptTitle(
        _ app: ghostty_app_t,
        target: ghostty_target_s,
        v: ghostty_action_prompt_title_e
      ) -> Bool {
