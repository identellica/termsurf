...
class PermissionRequest {
  ...
  static func show(
    ...
    if let window = window {
      alert.beginSheetModal(for: window) { response in
        handleResponse(
          response, rememberDecision: checkbox?.state == .on, key: key,
          allowDuration: allowDuration, rememberDuration: rememberDuration, completion: completion)
      }
    } else {
      let response = alert.runModal()
      handleResponse(
        response, rememberDecision: checkbox?.state == .on, key: key, allowDuration: allowDuration,
        rememberDuration: rememberDuration, completion: completion)
    }
  }
  ...
  private static func handleResponse(
    ...
    if rememberDecision, let rememberDuration = rememberDuration {
      ...
      case .forever:
        ...
        storeResult(result, for: key, duration: .seconds(3_153_600_000))
  ...
  private static func getStoredResult(for key: String) -> Bool? {
    ...
    guard let data = userDefaults.data(forKey: key),
      let storedPermission = try? NSKeyedUnarchiver.unarchivedObject(
        ofClass: StoredPermission.self, from: data)
    else {
      return nil
    }
  ...
  private static func storeResult(_ result: Bool, for key: String, duration: Duration) {
    ...
    if let data = try? NSKeyedArchiver.archivedData(
      withRootObject: storedPermission, requiringSecureCoding: true)
    {
      let userDefaults = UserDefaults.standard
