...
struct CommandPaletteView: View {
  ...
  var body: some View {
    let scheme: ColorScheme =
      if OSColor(backgroundColor).isLightColor {
        .light
      } else {
        .dark
      }
    ...
    VStack(alignment: .leading, spacing: 0) {
      CommandPaletteQuery(query: $query, isTextFieldFocused: _isTextFieldFocused) { event in
        ...
        case .move(.down):
          ...
          selectedIndex =
            (current >= UInt(filteredOptions.count - 1))
            ? 0
            : current + 1
...
private struct CommandPaletteQuery: View {
  ...
  init(
    query: Binding<String>, isTextFieldFocused: FocusState<Bool>,
    ...
    Button(action: action) {
      ...
      .overlay(
        RoundedRectangle(cornerRadius: 5)
          .strokeBorder(
            Color.accentColor.opacity(option.emphasis && !isSelected ? 0.3 : 0), lineWidth: 1.5)
      )
      .cornerRadius(5)
    }
