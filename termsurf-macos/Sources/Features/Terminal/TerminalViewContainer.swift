...
class TerminalViewContainer<ViewModel: TerminalViewModel>: NSView {
  ...
  init(ghostty: Ghostty.App, viewModel: ViewModel, delegate: (any TerminalViewDelegate)? = nil) {
    ...
    self.terminalView = NSHostingView(
...
extension TerminalViewContainer {
    ...
    fileprivate func addGlassEffectViewIfNeeded() -> NSGlassEffectView? {
  ...
  fileprivate func updateGlassEffectIfNeeded() {
    #if compiler(>=6.2)
      ...
      let backgroundColor =
        ...
        ?? NSColor(derivedConfig.backgroundColor)
      effectView.tintColor =
        backgroundColor
        .withAlphaComponent(derivedConfig.backgroundOpacity)
      if let window, window.responds(to: Selector(("_cornerRadius"))),
        let cornerRadius = window.value(forKey: "_cornerRadius") as? CGFloat
      {
  ...
  fileprivate func updateGlassEffectTopInsetIfNeeded() {
    #if compiler(>=6.2)
      guard #available(macOS 26.0, *), derivedConfig.backgroundBlur.isGlassStyle else {
        return
      }
  ...
  fileprivate struct DerivedConfig: Equatable {
