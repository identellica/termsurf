...
class TitlebarTabsVenturaTerminalWindow: TerminalWindow {
  ...
  private var hasWindowButtons: Bool {
    ...
    let miniaturizeIsHidden =
      standardWindowButton(.miniaturizeButton)?.isHiddenOrHasHiddenAncestor ?? true
  ...
  override func awakeFromNib() {
    ...
    titlebarColor = derivedConfig.backgroundColor.withAlphaComponent(
  ...
  override func syncAppearance(_ surfaceConfig: Ghostty.SurfaceView.DerivedConfig) {
    ...
    if let preferredBackgroundColor {
      appearance =
        (preferredBackgroundColor.isLightColor
    ...
    if titlebarTabs && styleMask.contains(.titled) {
      guard
        let tabBarAccessoryViewController = titlebarAccessoryViewControllers.first(where: {
          $0.identifier == Self.tabBarIdentifier
        })
      else { return }
  ...
  private func updateNewTabButtonOpacity() {
    guard
      let newTabButton: NSButton = titlebarContainer?.firstDescendant(
        withClassName: "NSTabBarNewTabButton") as? NSButton
    else { return }
    guard
      let newTabButtonImageView = newTabButton.firstDescendant(withClassName: "NSImageView")
        as? NSImageView
    else { return }
  ...
  private func updateNewTabButtonImage() {
    ...
    let imageLayer =
      newTabButtonImageLayer ?? VibrantLayer(forAppearance: isLightTheme ? .light : .dark)!
    ...
    newTabButton.layer?.sublayers?.first(where: { $0.className == "VibrantLayer" })?
  ...
  private func updateTabsForVeryDarkBackgrounds() {
    ...
    if let tabGroup = tabGroup, tabGroup.isTabBarVisible {
      guard
        let activeTabBackgroundView = titlebarContainer.firstDescendant(
          withClassName: "NSTabButton")?.superview?.subviews.last?.firstDescendant(
            withID: "_backgroundView")
      else { return }
  ...
  private func hideToolbarOverflowButton() {
    ...
    toolbarView.subviews.first(where: { $0.className == "NSToolbarClippedItemsIndicatorViewer" })?
      .isHidden = true
  }
  ...
  private func pushTabsToTitlebar(_ tabBarController: NSTitlebarAccessoryViewController) {
    ...
    DispatchQueue.main.async { [weak self] in
      ...
      accessoryClipView.leftAnchor.constraint(equalTo: windowButtonsBackdrop.rightAnchor).isActive =
        true
  ...
  private func addWindowButtonsBackdrop(titlebarView: NSView, toolbarView: NSView) {
    ...
    view.rightAnchor.constraint(
      equalTo: toolbarView.leftAnchor, constant: hasWindowButtons ? 78 : 0
    ).isActive = true
  ...
  var isHighlighted: Bool = true {
    didSet {
      ...
      if isLightTheme {
        ...
        let systemOverlayColor = NSColor(
          cgColor: CGColor(genericGrayGamma2_2Gray: 0.0, alpha: 0.45))!
        let titlebarBackgroundColor = terminalWindow.titlebarColor.blended(
          withFraction: 1, of: systemOverlayColor)

        let highlightedColor =
          terminalWindow.hasVeryDarkBackground ? terminalWindow.backgroundColor : .clear
        let backgroundColor =
          terminalWindow.hasVeryDarkBackground ? titlebarBackgroundColor : systemOverlayColor

        overlayLayer.isHidden = true
        layer?.backgroundColor =
          isHighlighted ? highlightedColor?.cgColor : backgroundColor?.cgColor
      }
    }
  }
...
private class CenteredDynamicLabel: NSTextField {
  ...
  override func draw(_ dirtyRect: NSRect) {
    guard
      let attributedString = self.attributedStringValue.mutableCopy() as? NSMutableAttributedString
    else {
