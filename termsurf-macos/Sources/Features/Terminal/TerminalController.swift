...
class TerminalController: BaseTerminalController, TabGroupCloseCoordinator.Controller {
  ...
  static func newTab(
    ...
    DispatchQueue.main.async {
      ...
      if !window.styleMask.contains(.fullScreen) && window.tabGroup?.windows.count ?? 1 == 1 {
        Self.lastCascadePoint = window.cascadeTopLeft(from: Self.lastCascadePoint)
      }
    ...
    let undoStates = tabGroup.windows
      .compactMap { tabWindow -> UndoState? in
        guard let controller = tabWindow.windowController as? TerminalController,
          var undoState = controller.undoState
        else { return nil }
  ...
  override func windowDidLoad() {
    ...
    if case .leaf(let view) = surfaceTree.root {
  ...
  @IBAction func closeOtherTabs(_ sender: Any?) {
    ...
    guard
      tabGroup.windows.contains(where: { window in
        // Ignore ourself
  ...
  @objc private func onMoveTab(notification: SwiftUI.Notification) {
    ...
    guard let selectedIndex = tabbedWindows.firstIndex(where: { $0 == selectedWindow }) else {
      return
    }
