  ...
  FullscreenDelegate
{
  ...
  func syncFocusToSurfaceTree() {
    for surfaceView in surfaceTree {
      ...
      let focused: Bool =
        (window?.isKeyWindow ?? false) && !commandPaletteIsShowing && focusedSurface != nil
  ...
  private func findNextFocusTargetAfterClosing(node: SplitTree<Ghostty.SurfaceView>.Node) -> Ghostty
    .SurfaceView?
  {
  ...
  @objc private func ghosttyDidFocusSplit(_ notification: Notification) {
    ...
    guard let directionAny = notification.userInfo?[Ghostty.Notification.SplitDirectionKey] else {
      return
    }
    ...
    guard
      let nextSurface = surfaceTree.focusTarget(
  ...
  @objc private func ghosttyDidResizeSplit(_ notification: Notification) {
    ...
    guard let amountAny = notification.userInfo?[Ghostty.Notification.ResizeSplitAmountKey] else {
      return
    }
    ...
    do {
      surfaceTree = try surfaceTree.resizing(
  ...
  @objc private func ghosttySurfaceDragEndedNoTarget(_ notification: Notification) {
    ...
    _ = TerminalController.newWindow(
      ghostty,
      tree: newTree,
      position: notification.userInfo?[Notification.Name.ghosttySurfaceDragEndedNoTargetPointKey]
        as? NSPoint,
      confirmUndo: false)
  }
  ...
  @objc private func onConfirmClipboardRequest(notification: SwiftUI.Notification) {
    ...
    guard let str = notification.userInfo?[Ghostty.Notification.ConfirmClipboardStrKey] as? String
    else { return }
    guard
  ...
  func clipboardConfirmationComplete(
    _ action: ClipboardConfirmationView.Action, _ request: Ghostty.ClipboardRequest
  ) {
