...
class QuickTerminalController: BaseTerminalController {
  ...
  init(
    ...
    self.screenStateCache = QuickTerminalScreenStateCache(
      stateByDisplay: restorationState?.screenStateEntries ?? [:])
  ...
  func animateIn() {
      ...
      let ghostty_app = ghostty.app
    {
      if let tree = restorationState?.surfaceTree, !tree.isEmpty {
        surfaceTree = tree
        let view =
          tree.first(where: { $0.id.uuidString == restorationState?.focusedSurface }) ?? tree.first!
        focusedSurface = view
  ...
  override func syncAppearance() {
    ...
    if !isBackgroundOpaque
      && (self.derivedConfig.backgroundOpacity < 1 || derivedConfig.backgroundBlur.isGlassStyle)
    {
