...
enum QuickTerminalPosition: String {
  ...
  func configuredFrameSize(on screen: NSScreen, terminalSize: QuickTerminalSize) -> NSSize {
    let dimensions = terminalSize.calculate(
      position: self, screenDimensions: screen.visibleFrame.size)
  ...
  func initialOrigin(for window: NSWindow, on screen: NSScreen) -> CGPoint {
    ...
    case .top:
      return .init(
        x: round(
          screen.visibleFrame.origin.x + (screen.visibleFrame.width - window.frame.width) / 2),
    ...
    case .left:
      return .init(
        ...
        y: round(
          screen.visibleFrame.origin.y + (screen.visibleFrame.height - window.frame.height) / 2))
