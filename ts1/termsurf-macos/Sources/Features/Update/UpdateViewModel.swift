...
class UpdateViewModel: ObservableObject {
  ...
  var description: String {
    ...
    case .installing(let install):
      return install.isAutoUpdate
        ? "Restart to Complete Update" : "Installing update and preparing to restart"
    case .notFound:
  ...
  var backgroundColor: Color {
    ...
    case .permissionRequest:
      return Color(
        nsColor: NSColor.systemBlue.blended(withFraction: 0.3, of: .black) ?? .systemBlue)
    case .updateAvailable:
      return .accentColor
    case .notFound:
      return Color(
        nsColor: NSColor.systemBlue.blended(withFraction: 0.5, of: .black) ?? .systemBlue)
    case .error:
...
enum UpdateState: Equatable {
  ...
  struct UpdateAvailable {
    ...
    var releaseNotes: ReleaseNotes? {
      ...
      return ReleaseNotes(
        displayVersionString: appcastItem.displayVersionString, currentCommit: currentCommit)
    }
  }
  ...
  enum ReleaseNotes {
    ...
    init?(displayVersionString: String, currentCommit: String?) {
      ...
      if let currentHash = currentCommit, !currentHash.isEmpty,
        let url = URL(
          string: "https://github.com/ghostty-org/ghostty/compare/\(currentHash)...\(newHash)")
      {
        self = .compareTip(url)
